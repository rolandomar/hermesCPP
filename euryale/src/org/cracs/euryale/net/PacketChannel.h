...
public:
    ...
    PacketChannel() :
    AbstractStreamChannel<SOCK_TYPE, SYNC_TYPE>(),
    ...
    AbstractStreamChannel<SOCK_TYPE, SYNC_TYPE>(asyncRead, asyncWrite, ec, reserve, qos, closeListener),
...
handleInputParsePacket(PACKET*& inputPacket, ACE_Time_Value* timeout) {
    ...
    for(int i=0; i<readSize; i++){
        printf("%d,",m_partialMessage->base()[i]);
    }
    printf("\n");
...
//            ACE_DEBUG((LM_DEBUG,
//                    ACE_TEXT("(%t|%T)CCCCCCCCCCCCcc TRACE: PacketChannel::handleInputParsePacket() - body < len buf=%d\n"),
//                   m_partialMessage->length()));
            return 0;
        } else {//complete                                    
            ACE_DEBUG((LM_DEBUG,
                    ACE_TEXT("(%t|%T)FFFFFFFFFFFFFFf TRACE: PacketChannel::handleInputParsePacket() - body < len buf=%d\n"),
                    m_partialMessage->length()));
            ...
            if (m_header->getBodySize() > readSize) {
                ...
                //         readSize,
                        m_partialMessage->space(),
                         (m_header->getBodySize() - readSize),m_header->getBodySize()
                        ));
